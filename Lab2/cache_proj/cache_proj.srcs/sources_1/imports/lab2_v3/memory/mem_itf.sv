...
(   input bit clk
    ...
    /* 
        Controller inputs/Device Outputs:
            mem_resp: indicates a successful mem_read
            mem_rdata: data from memory 

        Controller outputs/Device Inputs:
            clk: Clock. Sent to device for synch.
            rst: ...
            mem_read: tell device to read
            mem_addr: give device the address.
            mem_wdata: data to write
            mem_byte_enable: strobe signal. Mask that says which bytes to write.  
    */
    modport device(
